...
| `rating`               | The age rating of the song.<br>1 = Family Friendly<br>2 = Supervision Recommended<br>3 = Mature Content<br>4 or 0 = Unrated | number     |
|                        |                                                                                  |           |
| Chart info             |                                                                                  |           |
...
| `version`              | Revision for the chart.                                                          | number    |
|                        |                                                                                  |           |
| `preview_start_time`   | Timestamp in milliseconds that a song preview should start at.                   | number    |
| `preview_end_time`     | Timestamp in milliseconds that the preview should stop at.                       | number    |
|                        |                                                                                  |           |
| Sorting metadata       |                                                                                  |           |
| `playlist`             | Key name used to group related songs together under a single playlist.<br>Defaults to the name of the containing folder | string    |
| `sub_playlist`         | Key name used to create a sub-playlist underneath the key specified in `playlist`. | string  |
|                        |                                                                                  |           |
| `album_track`          | The order to sort the song in when sorting by album.                             | number    |
| `track`                | Same as `album_track`. (Prioritize `album_track` if both are present).           | number    |
| `playlist_track`       | The order to sort the song in when sorting by playlist.                          | number    |
|                        |                                                                                  |           |
| `song_length`          | Length of the song's audio in milliseconds.<br>Metadata only, don't rely on this. | number   |
| `modchart`             | Indicates whether this song is a modchart, for filtering purposes.               | boolean   |
|                        |                                                                                  |           |
| Miscellaneous          |                                                                                  |           |
...
| Tag Name               | Description                                                                      | Data type |
| :-------               | :----------                                                                      | :-------: |
| `lyrics`               | Indicates whether the song has lyrics for filtering purposes.<br>This should be detected automatically by the game instead. | boolean   |
|                        |                                                                                  |           |
...
| `star_power_note`      | Same as `multiplier_note`.                                                       | MIDI note |
